...
public class SpawnCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (args.length == 0) {
			...
			int rn = r.nextInt(4);
			if (rn == 0) {
				final Location loc = Manager.pegarLocation("SPAWN").clone().add(0.0D, 0.0D, 2.0D);
				if (!p.hasPermission("wandy.semdelay")) {
					p.sendMessage("§eTeleportando em 3 segundos...");
					Bukkit.getScheduler().scheduleSyncDelayedTask(Main.plugin, new Runnable() {
						public void run() {
							p.teleport(loc);
							EntrarListener.mandarAction(p, "§eTeletransportado para o SPAWN com sucesso!");
						}
					}, 60L);
					return true;
				}
			...
			if (rn == 1) {
				final Location loc = Manager.pegarLocation("SPAWN").clone().subtract(2.0D, 0.0D, 0.0D);
				if (!p.hasPermission("wandy.semdelay")) {
					p.sendMessage("§eTeleportando em 3 segundos...");
					Bukkit.getScheduler().scheduleSyncDelayedTask(Main.plugin, new Runnable() {
						public void run() {
			...
			if (rn == 2) {
				final Location loc = Manager.pegarLocation("SPAWN").clone().subtract(0.0D, 0.0D, 2.0D);
				if (!p.hasPermission("wandy.semdelay")) {
					p.sendMessage("§eTeleportando em 3 segundos...");
					Bukkit.getScheduler().scheduleSyncDelayedTask(Main.plugin, new Runnable() {
						public void run() {
			...
			if (rn == 3) {
				final Location loc = Manager.pegarLocation("SPAWN").clone().add(2.0D, 0.0D, 0.0D);
				if (!p.hasPermission("wandy.semdelay")) {
					p.sendMessage("§eTeleportando em 3 segundos...");
					Bukkit.getScheduler().scheduleSyncDelayedTask(Main.plugin, new Runnable() {
						public void run() {
		...
		if (args.length == 1) {
			if (args[0].equalsIgnoreCase("setar")) {
				if (p.hasPermission("wandy.setarspawn")) {
					...
					if (Manager.checarLocExiste("SPAWN")) {
						Manager.updateLocation("SPAWN", loc);
						p.sendMessage("§aSpawn setado com sucesso.");
						return true;
					}
					Manager.setarLocation("SPAWN", loc);
					p.sendMessage("§aSpawn setado com sucesso.");
					return true;
				}
				if (!Manager.checarLocExiste("SPAWN")) {
